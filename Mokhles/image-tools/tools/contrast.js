document.addEventListener('DOMContentLoaded', () => {
    ...
    downloadBtn.addEventListener('click', downloadContrasted);
    ...
    function updateContrastPreview() {
        const level = contrastSlider.value;
        contrastValueSpan.textContent = `${level}%`;
        
        // Use fast CSS filter for real-time preview
        previewImage.style.filter = `contrast(${level}%)`;
    }

    function downloadContrasted() {
        const level = contrastSlider.value;
        ...
        const newName = `contrast_${level}_${currentFile.name}`;
